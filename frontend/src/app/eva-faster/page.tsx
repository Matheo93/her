  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (ttsResponse.ok) {
        ...
        const videoResponse = await fetch(`${FLP_API}/animate_with_audio`, {
          method: "POST",
          body: formData,
        });
          ...
          <div className="p-4 border-t border-gray-700">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-gray-800 text-white rounded-full px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
              />
              <button
                onClick={sendMessage}
                disabled={!isPrepared || isLoading || !input.trim()}
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed px-6 py-3 rounded-full font-medium transition-all"
              >
                {isLoading ? "..." : "Envoyer"}
              </button>
            </div>
