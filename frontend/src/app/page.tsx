...
export default function Home() {
    ...
    const connect = () => {
      ...
      ws.onmessage = async (event) => {
        ...
        switch (data.type) {
          ...
          case "response_end":
            setCurrentResponse((prev) => {
              if (prev) {
                setMessages((msgs) => [
                  ...msgs,
                  { role: "assistant", content: prev, emotion: currentEmotion },
                ]);
              }
              return "";
            });
          ...
          case "transcript":
            // STT result from mic
            setMessages((msgs) => [...msgs, { role: "user", content: data.text }]);
            setIsLoading(true);
            setCurrentResponse("");
            break;
  ...
  }, [selectedVoice, voiceEnabled, playNextAudio, currentEmotion]);
                  ...
                  <div className="p-2">
                    ...
                    {voices.map((v) => (
                      ...
                      >
                        ...
                        {v.id === "male-deep" && "🎭 Adam (Profond)"}
                        {v.id === "eva-en" && "🇬🇧 Eva (English)"}
                        {v.id === "eva-en-warm" && "🇺🇸 Eva (US English)"}
