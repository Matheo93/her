...
export function RealtimeVoiceCall({
  ...
  const stopRecordingAndSend = useCallback(async () => {
    ...
    return new Promise<void>((resolve) => {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Send to backend
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN && blob.size > 1000) {
  ...
  const handleSpeechStart = useCallback(() => {
    ...
    startRecording();
  }, [callState, sendInterrupt, startRecording]);
  ...
  // Initialize VAD
  const vad = useVAD({
    threshold: 0.015, // Slightly higher threshold to avoid false positives
    minSpeechDuration: 150, // Quick speech detection
    silenceTimeout: 600, // Wait 600ms of silence before ending
    minSilenceDuration: 300, // Confirm silence after 300ms
