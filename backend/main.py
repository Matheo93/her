        ...
        while True:
            ...
            if "text" in msg:
                ...
                if msg_type == "config":
                    ...
                    if voice not in VOICES:
                        voice = DEFAULT_VOICE
                    await ws.send_json({"type": "config_ok", "voice": voice, "auto_mood": auto_mood})
                    continue
                    ...
                    if auto_mood:
                        ...
                        await ws.send_json({"type": "mood", "mood": current_mood})
                    ...
                    for sentence, task in tts_tasks:
                        try:
                            audio = await task
                            if audio:
                                await ws.send_json({"type": "audio_start", "sentence": sentence[:50]})
                                await ws.send_bytes(audio)
                        except Exception as e:
    ...
    try:
        while True:
            ...
            if "text" in msg:
                ...
                # INTERRUPT: User wants to stop Eva speaking
                    ...
                    if session.is_speaking:
                        session.interrupt()
                        await ws.send_json({"type": "speaking_end", "reason": "interrupted"})
                    continue
                ...
                # User started speaking (interrupt Eva)
                    ...
                    if session.is_speaking:
                        session.interrupt()
                        await ws.send_json({"type": "speaking_end", "reason": "user_speaking"})
                    continue
                ...
                # Text message from user
                    ...
                    if session.is_speaking:
                        session.interrupt()
                        await ws.send_json({"type": "speaking_end", "reason": "new_message"})
                    ...
                    quick_response = response_cache.get(content)
                    ...
                    if quick_response:
                        ...
                        await ws.send_json({"type": "token", "content": quick_response})
                        ...
                        try:
                            ...
                            if audio_data and not session.is_interrupted:
                                await ws.send_json({"type": "audio_chunk", "size": len(audio_data)})
                                await ws.send_bytes(audio_data)
                        except Exception as e:
                            print(f"TTS error: {e}")

                        await ws.send_json({"type": "response_end", "text": quick_response})
                        session.is_speaking = False
                        await ws.send_json({"type": "speaking_end", "reason": "complete"})
                        session.reset()
                        continue

                    # NORMAL PATH: Stream LLM then TTS
                    full_response = ""

                    # Collect full LLM response first (faster than sentence-by-sentence TTS)
                    async for token in stream_llm(sid, content):
            ...
            # Handle binary audio (voice input)
                ...
                if session.is_speaking:
                    session.interrupt()
                    await ws.send_json({"type": "speaking_end", "reason": "voice_input"})
                ...
                # FAST PATH: Check for cached quick response first
                quick_response = response_cache.get(text)
