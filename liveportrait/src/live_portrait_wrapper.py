...
class LivePortraitWrapper(object):
    ...
    def inference_ctx(self):
        if self.device == "mps":
            ...
            # Handle DirectML (privateuseone) and other non-string devices
            device_str = str(self.device) if not isinstance(self.device, str) else self.device
            if 'privateuseone' in device_str or 'cpu' in device_str:
                # DirectML or CPU - no autocast
                ctx = contextlib.nullcontext()
            else:
                ctx = torch.autocast(device_type=device_str[:4], dtype=torch.float16,
